...
##pull CPM data		
if sys.argv[1] == "saatchi" or sys.argv[1] == "na":
	for x in campaign_ids:
		url = "https://api.mediamath.com/reporting/v1/std/performance_aggregated?dimensions=campaign_id&metrics=total_spend_cpm&filter=campaign_id="+x+"&time_rollup=all&time_window=campaign_to_date"
		r_auth = requests.request("POST", login, data=payload, headers=headers)
		c = r_auth.cookies
		r_camp = requests.request("GET", url, headers=headers2, cookies=c)
		r = r_camp.content
		...
		#cpm = round(float(re.sub('[^0-9.]','', cpm)),2)
		
		#df.loc["415309",'T1 CPM (CTD)'] = cpm
...
def red(val):
	...
	return ['color: white','color: %s' % color]	
...
#da = df.style.apply(highlight_budget, axis=1,subset=['Smartsheet Budget','T1 Budget']).set_properties(**{'background-color': 'black',
#                           'border-color': 'lawngreen'})

pd.option_context('display.precision', 2)
da = df.style.apply(red, subset=['Smartsheet CPM', 'T1 CPM (CTD)'],axis=1).apply(green, subset=['Smartsheet Budget', 'T1 Budget'],axis=1).set_properties(**{'background-color': 'black','max-width': '150px'}).set_table_attributes('border="1" class="dataframe table table-hover table-bordered"')
