...
function get_pmp(strat,callback){
...
$("#punch").click(function() {
	feedback = "";
	var strat_list_update = [];
	strat_list_update = get_selected_strats();
	strat_list_update = strat_list_update[0]; 
		
for(var i=0; i<strat_list_update.length; i++) {
	var current_strat = strat_list_update[i];
 	get_pmp(current_strat,function(current_strat,deals){
		add_pmp(current_strat,deals,function(deal_success){
		if (deal_success == 1) {
		feedback = feedback + "Success on " + current_strat + ": Deals Added</p>";								
