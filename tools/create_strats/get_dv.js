...
function get(strat, callback){
	...
	console.log(con);
	var request = $.ajax({
		url: "https://adroit-tools.mediamath.com/t1/api/v2.0/strategies/"+strat+"/targeting_segments?full=*&with=strategy,campaign,advertiser",
		type: "GET",
		cache: false,
		dataType: "xml",
		...
		success: function(xml) {
			...
			var strat_id = entry.find("entity").attr("id");
			var strat_name = entry.find("entity").attr("name");
			console.log(strat_id);
			
			var camp_id = entry.find("entity").find("entity").attr("id");
			var camp_name = entry.find("entity").find("entity").attr("name");
			
			var adv_id = entry.find("entity").find("entity").find("entity").attr("id");
			var adv_name = entry.find("entity").find("entity").find("entity").attr("name");
			
			var info = adv_id + "," + adv_name + "," + camp_id + "," + camp_name + "," + strat_id + "," + strat_name;
...
function get_button(d,numRows){
	...
	var success = 0; 
	var info = "";
	var header = "adv_id,adv_name,camp_id,camp_name,strat_id,strat_name,dv_inc_op,dv_inc_id,ia_inc_op,ia_inc_id,pe_inc_op,pe_inc_id,gs_inc_op,gs_inc_id,sm_inc_op,sm_inc_id,ctx_exc_id";
...
function dropdown(){
	...
	var success = 0; 
	var info = "";
	var header = "adv_id,adv_name,camp_id,camp_name,strat_id,strat_name,dv_inc_op,dv_inc_id";
