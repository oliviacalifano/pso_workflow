...
//doesn't add the same day part again
...
function get_current_daypart(strat_id, callback){
	...
	if(allparts == false){
	var new_user_time = $('input[name=user_time]:checked', '#user_time').val();
	var new_start_hour = $("#daypart_start").val();
	var new_end_hour = $("#daypart_end").val();
	var new_days = get_selected_days().join(""); 
	}
	...
	var request = $.ajax({
		...
		success: function(xml) {
			...
			var new_daypart = "day_parts."+ num + ".user_time="+new_user_time+"&day_parts."+ num + ".start_hour="+new_start_hour+"&day_parts."+ num + ".end_hour="+new_end_hour+"&day_parts."+ num + ".days="+new_days;	
	
			console.log(hold);
			console.log(test);
			console.log(new_daypart);
	
		
			if(hold == test || hold == new_daypart)
			{
				console.log("yes");
				num = num - 1;
			}
