...
function get_selected_eventPixels() 
{

	var eventPixels_id = []; 
	
	//get selected ctxlice ids
	$("#eventPixels_list").each(function() { 
		eventPixels_id.push($(this).val()); 
	});
	console.log(eventPixels_id);
	
	return eventPixels_id[0];

}

function get_eventPixel_targets(strat, operation, pool, log, opPixels, callback){
	
	//console.log("getting event pixels for: ",strat_id);
...
function update_eventPixel_targeting() {
	...
	if (typeof include_exclude === 'undefined') { return; }
		...
		change = get_selected_eventPixels(); 
		console.log("list of event pixel targets", change);

		console.log("starting to loop through strats and update event pixels");
		for(var i=0; i<strat_list.length; i++) {
			...
			if(change !== null){
				get_eventPixel_targets(current_strat, add_remove, include_exclude, and_or, change, function(current_strat, v, update) 
				{
