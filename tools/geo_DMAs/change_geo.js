...
function get_dma_geo_targets(strat_id, callback){
	...
	var request = $.ajax({
		url: "https://adroit-tools.mediamath.com/t1/api/v2.0/strategies/"+strat_id+"/target_dimensions/1",
		type: "GET",
		cache: false,
		dataType: "xml",
...
function update_geo_targeting() {
	...
	if (typeof include_exclude === 'undefined') { return; }
		...
		for(var i=0; i<mod_geo.length; i++){
			...
			if (mod_geo[i][0] == 'D'){
				mod_dma.push(mod_geo[i].substring(1));
			}
			
		}

		console.log("dmas: ", mod_dma);
		...
		for(var i=0; i<strat_list.length; i++) {
			...
			get_dma_geo_targets(current_strat, function(current_strat, final_list) 
			{
				console.log("DMA list for current_strat.  Include: ", final_list['include'], " Exclude: ", final_list['exclude']); 
				if(add_remove == 'add'){
					final_list[include_exclude] = add_to_final_list(final_list[include_exclude], mod_dma);
				}
				if(add_remove == 'remove'){
					final_list[include_exclude] = remove_from_final_list(final_list[include_exclude], mod_dma);
				}
				console.log("After..DMA list for current_strat.  Include: ", final_list['include'], " Exclude: ", final_list['exclude']);
				
				set_targeting(current_strat, '1', final_list, function(success)
				{
					if (success == 1 && mod_dma.length!=0) {
						feedback = feedback + "<p>Updated DMA targeting for "+current_strat+". Check changes <a target=\"_blank\" href=\"https://adroit-tools.mediamath.com/t1/api/v2.0/strategies/"+current_strat+"/target_dimensions/1\">here</a></p>";								
						$("#feedback").html(feedback); 
					}
					else{
						var error = "ERROR: ";
						feedback = feedback + "<p>" + error.fontcolor("red")+current_camp+". Check changes <a target=\"_blank\" href=\"https://adroit-tools.mediamath.com/t1/api/v2.0/strategies/"+current_strat+"/target_dimensions/1\">here</a></p>";								
						$("#feedback").html(feedback);
						}
				});
			});
