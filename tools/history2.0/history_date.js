...
function get_history(strat, n, col, camp_n, camp_i, start_date, end_date, callback){
	
	var csv="";

	var request = $.ajax({
		url: "https://adroit-tools.mediamath.com/t1/api/v2.0/"+col+"/"+strat+"/history?q=created_on>"+start_date+"%26created_on<"+end_date,
		type: "GET",
		cache: false,
		dataType: "xml",
		...
		success: function(xml) {
			...
			if(col == "strategies"){
			var header = "date,time,campaign_id,campaign_name,strategy_id,strategy_name,user_id,user_name,type,old_value,new_value";
			}
			else {
			var header = "date,time,campaign_id,campaign_name,user_id,user_name,type,old_value,new_value";
			}
...
function download() {
	...
	var start = $('#reportrange').data('daterangepicker').startDate.format('YYYY-MM-DDT00:00:00');
	var end = $('#reportrange').data('daterangepicker').endDate.format('YYYY-MM-DDT23:59:59');
	...
	if(collection == 'campaigns'){
		...
		get_camp_info(current_strat,function(current_strat,name){
			get_history(current_strat, name, collection, "", "", start, end, function(current_strat, doc, header) 
			{	counter++;
				console.log(counter);
				console.log(header);
				console.log(head);
	...
	if(collection == 'strategies'){
		...
		get_strat_info(current_strat,function(current_strat,name,camp_name,camp_id){
			get_history(current_strat, name, collection, camp_name, camp_id, start, end, function(current_strat, doc, header) 
			{	counter++;
				console.log(counter);
				console.log(header);
				console.log(head);
