...
function get_ias_targets(strat_id, callback){
	...
	var request = $.ajax({
		...
		//if strat is running on all exchanges, assign 1 and end function
			...
			callback(strat_id,final_list,op_ia,op_pe,op_dv)
...
function set_targeting(strat_id, final_list, ia, pe, dv, callback)
{		
		var include_array = [];
		var exclude_array = [];
		var include_list = "";
		...
		var sum = in_pe_length + in_ia_length + in_dv_length+ ex_ia_length;
		console.log("sum: " + sum);
		...
		for(var i = 0; i<final_list['include_ia'].length; i++){
			include_array.push("segments." +sum+".id="+final_list['include_ia'][i]+"&segments."+sum+".restriction=INCLUDE"+"&segments."+sum+".operator="+ia+"&segments."+sum+".user_cpm=0.00");
			sum = sum -1;
		}
...
function update_ias_targeting() {
	...
	return; }
		...
		for(var i=0; i<strat_list.length; i++) {
			...
			if(mod_ias !== null){
				get_ias_targets(current_strat, function(current_strat, final_list, ia_op, pe_op, dv_op) 
				{
					console.log(ia_op);
					if(logic_ia != "CURRENT"){
						ia_op = logic_ia;
