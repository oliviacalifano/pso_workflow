	...
	var request = $.ajax({
		...
		success: function(csv) {
			//console.log(csv);
					...
					if(metric == "media_cost,impressions"){	
...
function checks(click){
	...
	var metric = $('input[name=metric]:checked', '#metric').val();
	...
	var doc = "";
		...
		check_todaySpend(current_camp, metric, function(today,t){
			check_yestSpend(current_camp, metric, function(yest,y) 
			{	
				console.log(today);
				console.log(yest);
				if(click == "plot"){
...
toolTip1 = ['Today', 'Yesterday'];
  plot2 = $.jqplot ('chart2', [to,ye],{	
      ...
      title: 'Cumulative Spend by Hour',
        legend: {
            show: true,
            placement: 'outside',
