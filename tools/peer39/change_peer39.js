...
function get_ctxl_targets(strat_id, callback){
	...
	var request = $.ajax({
		...
		//if strat is running on all exchanges, assign 1 and end function
			...
			callback(strat_id,final_list,op_pe,op_ia)
...
function set_targeting(strat_id, final_list, pe, ia, callback)
{		
		var include_array = [];
		var exclude_array = [];
		var include_list = "";
		var include_list_ias = "";
		var exclude_list = "";
		var success = 0;
		...
		var sum = in_pe_length + in_ias_length + ex_pe_length;
		console.log("sum: " + sum);
...
function update_ctxl_targeting() {
	...
	return; }
		...
		for(var i=0; i<strat_list.length; i++) {
			...
			if(mod_ctxl !== null){
				get_ctxl_targets(current_strat, function(current_strat, final_list, pe_op, ia_op) 
				{
					console.log(pe_op);
					if(logic_pe != "CURRENT"){
						pe_op = logic_pe;
					...
					set_targeting(current_strat, final_list, pe_op, ia_op, function(success)
					{			
