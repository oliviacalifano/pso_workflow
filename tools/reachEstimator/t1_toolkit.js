...
function push_entities_to_multiselect2(xml, id)
{
	options = {};
...
function push_entities_to_multiselect_with_name_prefix(xml, id)
{
	options = [];
	...
	$(xml).find('entity').each(function(result) {
		item_id = $(this).attr('id')
		name = $(this).attr('name')
		options.push("<option value="+item_id+">"+item_id+" - "+name+"</option>")
	});
	console.log(options, id);
	$(id).append(options);
	$(id).multipleSelect({
