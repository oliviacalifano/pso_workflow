...
function get_strat_info(strat,callback){
	...
	var request = $.ajax({
		...
		success: function(xml) {
			...
			a.push(i.find("prop[name=supply_type]").attr("value"));
			...
			a.push(i.find("prop[name=min_bid]").attr("value"));
...
function get_geos(strat, s, callback){
	...
	var request = $.ajax({
		...
		success: function(xml) {
			...
			//only included geo
			var entry = $(xml).find("include").find("entity");
			entry.each(function(){
...
function download() {
	...
	var header = "strat_id,name,campaign_id,status,supply_type,media_type,budget,description,type,use_campaign_start,use_campaign_end,start_date,end_date,bid_price_is_media_only,run_on_all_exchanges,supply_source_id,frequency_type,frequency_amount,frequency_interval,use_optimization,goal_type,goal_value,min_bid,max_bid,pacing_type,pacing_amount,pacing_interval,pixel_target_expr,roi_target,version,geo_region,concept_id,aud_inc_id,aud_exc_id,ctx_op,contextual_id,tech_inc_id,tech_exc_id";
