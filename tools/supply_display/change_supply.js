...
function remove_supply(supply_list, mod_supplies, callback) {
	...
	if (supply_list == '1') {
		...
		var request = $.ajax({
			url: "https://adroit-tools.mediamath.com/t1/api/v2.0/supply_sources?sort_by=name&q=rtb_type%21%3DBATCH%26has_display%3D%3D1%26status%3D%3D1%26rtb_enabled%3D%3D1",
			type: "GET",
			cache: false,
			dataType: "xml",
			success: function(xml) {
